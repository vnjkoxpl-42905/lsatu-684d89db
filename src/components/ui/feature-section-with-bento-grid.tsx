...
const FeatureCard = ({
  ...
  return (
    <div className={cn("flex flex-col lg:border-r py-10 relative group/feature dark:border-neutral-800", className)}>
      {children}
    </div>
  );
};
...
const FeatureTitle = ({ children }: { children?: React.ReactNode }) => {
  return (
    <p className="max-w-5xl mx-auto text-left tracking-tight text-foreground text-xl md:text-2xl md:leading-snug px-8">
      {children}
    </p>
  );
};
...
const FeatureDescription = ({ children }: { children?: React.ReactNode }) => {
  return (
    <p className={cn("text-sm md:text-base max-w-4xl text-left mx-auto text-muted-foreground font-normal px-8")}>
      {children}
    </p>
  );
};
      ...
      <div className="flex flex-row -ml-20">
        {images.map((image, idx) => (
          <motion.div
            ...
            whileTap="whileTap"
            className="rounded-xl -mr-4 mt-4 p-1 bg-card border border-border flex-shrink-0 overflow-hidden"
          >
            <img
              src={image}
              alt="bento images"
