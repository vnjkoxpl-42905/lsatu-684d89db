...
import { roleQuestions } from './data';

type QuizState = 'test' | 'teardown';
...
export default function AdvancedQuizViewer({ onComplete }: { onComplete?: (count: number) => void }) {
  ...
  const [state, setState] = useState<QuizState>('test');
  ...
  if (finished) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <h2 className="text-xl font-bold text-foreground">All 6 Questions Complete</h2>
        ...
        <button
          onClick={() => { setCurrentIndex(0); setState('test'); setSelectedAnswer(null); setFinished(false); setCompletedCount(0); }}
              ...
              <div className="rounded-xl border border-border bg-card p-6 space-y-4">
                ...
                {question.answerBreakdowns.map(ab => (
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed pl-6">{ab.explanation}</p>
                  </div>
                ))}
              </div>
