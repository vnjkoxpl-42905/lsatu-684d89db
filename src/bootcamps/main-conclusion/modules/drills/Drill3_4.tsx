...
import { DRILL_3_4_STAGE_1, type FamilyQuestion } from '@/bootcamps/main-conclusion/content/drills.source';
...
const stages: Stage<FamilyQuestion>[] = [
  ...
  { number: 2, title: 'Rebuttal-only set', hint: 'Stage 2 authors at C.10.', questions: [] },
    ...
    <article className="px-6 py-10 max-w-3xl mx-auto space-y-4">
      ...
      {completed ? (
        <Card variant="elev" className="border-l-4 border-l-[rgb(var(--success)/0.50)]">
          ...
          <Button onClick={() => navigate('/bootcamp/structure-v2/simulator')} className="mt-3">
            Go to Simulator →
          </Button>
        </Card>
      ) : null}
